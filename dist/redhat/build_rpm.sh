...
sudo mock "${MOCK_OPTS[@]}" --rootdir="$(pwd)/build/redhat/mock" --buildsrpm --root="$TARGET" --resultdir="$(pwd)/build/redhat/srpms" --spec="dist/redhat/$DRIVER_NAME.spec" --sources="build/redhat/sources/$DRIVER_NAME-$DRIVER_VERSION-$DRIVER_RELEASE.tar"
...
sudo mock "${MOCK_OPTS[@]}" --rootdir="$(pwd)/build/redhat/mock" --enable-network --rebuild --root="$TARGET" --resultdir="$(pwd)/build/redhat/rpms" "build/redhat/srpms/$SRPM"
